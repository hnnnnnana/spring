...
<div>
	...
	<c:if test="${!empty loginInfo }">
	${loginInfo.name }님, 안녕하세요. <!-- getName 메소드 호출 -->
...
<div> ${boardVo.page }/ ${totPage }</div> <%-- ${boardVo.page } -> 커맨드객체일 경우에는 알아서 set해준다 -> 첫자를 소문자로 바꿔서 이름을 넣어준다.--%>
<table border="1">
	<tr>
		<th>번호</th>
		<th>
			<c:choose>
				...
				<c:otherwise>
					<a href="index.do?orderCond=title_asc">제목</a> <!-- 클릭하면 발생되는 경로 -->
				</c:otherwise>
			</c:choose>
		</th>
		<th>
	...
	<c:forEach var="vo" items="${list}"> <!-- var : variable, 변수 -->
	<tr>
		<td>${vo.boardno }</td>
